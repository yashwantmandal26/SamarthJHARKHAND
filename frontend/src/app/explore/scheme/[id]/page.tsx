...
export default function SchemeDetailPage() {
  ...
  useEffect(() => {
    ...
    async function fetchScheme() {
      try {
        const res = await fetch(`http://localhost:8000/api/schemes/${id}`);
        if (res.ok) {
  ...
  if (!scheme) {
    return (
      <div className="flex justify-center items-center h-screen flex-col">
        ...
        <Link href="/explore" className="text-orange-500 underline">Return to Explore</Link>
      </div>
    );
  }
    ...
    <div className="flex-1 w-full max-w-5xl mx-auto px-4 sm:px-6 py-20 page-enter">
      ...
      <nav className="mb-8 text-sm font-medium">
        <ol className="list-none p-0 inline-flex items-center text-slate-400">
          <li className="flex items-center">
            <Link href="/explore" className="text-orange-500 hover:text-orange-400">Explore</Link>
      ...
      <div className="relative glass-card rounded-3xl p-8 sm:p-12 mb-12 overflow-hidden border-orange-500/20">
        ...
        <div className="relative z-10">
          ...
          <h1 className="text-3xl sm:text-5xl font-bold text-white mb-3 tracking-tight">{scheme.name}</h1>
          <h2 className="text-xl sm:text-2xl text-orange-400 font-hindi mb-6">{scheme.name_hindi}</h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <section className="glass p-8 rounded-3xl border-slate-800">
             <h3 className="text-2xl font-bold mb-6 flex items-center gap-3">
               <span className="text-orange-500">🎯</span> Eligibility Criteria
             </h3>
          ...
          <section className="glass p-8 rounded-3xl border-slate-800">
             <h3 className="text-2xl font-bold mb-6 flex items-center gap-3">
               <span className="text-orange-500">📄</span> Required Documents
             </h3>
             ...
             <p className="text-xs text-rose-400 mt-4">* mandatory document</p>
          </section>
        ...
        <div className="space-y-8">
          ...
          <section className="glass-card p-6 md:p-8 rounded-3xl border-emerald-500/20 bg-emerald-500/5 text-center relative overflow-hidden">
             ...
             <h3 className="text-sm font-bold text-emerald-500 uppercase tracking-widest mb-4">Benefit Type: {scheme.benefits?.type?.replace('_', ' ')}</h3>
             ...
             {scheme.benefits?.amount ? (
                ...
                <div className="text-3xl md:text-4xl font-bold text-white mb-2">
                  Up to ₹{scheme.benefits.max_amount.toLocaleString()}
                </div>
             ) : null}
          ...
          <section className="glass p-6 md:p-8 rounded-3xl border-slate-800 relative">
             <h3 className="text-xl font-bold mb-6 text-white border-b border-slate-800 pb-4">How to Apply</h3>
             <div className="space-y-6">
                
                {scheme.application_process?.online_url && (
                   <div>
                     <p className="text-xs font-bold text-slate-500 uppercase mb-2">Online Portal</p>
                     <a 
                ...
                <div>
                   <p className="text-xs font-bold text-slate-500 uppercase mb-4">Manual Steps</p>
